...
// SVG Component để tránh re-render không cần thiết
const PlusIcon = () => (
  <svg
...
// Danh sách menu
const MENU_ITEMS = [
  { icon: <PlusIcon />, title: "Invite", to: "/login" },
  { icon: <SettingsIcon />, title: "Preferences", to: "/login" },
];
            ...
            {MENU_ITEMS.map((menu, index) => (
              <RadixDropdownMenu.Item key={index} className="cursor-pointer text-sm text-popover-foreground hover:bg-accent rounded-sm px-2 py-1.5">
                <Link to={menu.to} className="text-white flex items-center">
                  <span className="mr-2 text-white">{menu.icon}</span>
